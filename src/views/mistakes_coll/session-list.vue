...
const Refresh=()=>{
    if(searchParams.value==""){
        API.get(`/api/data/mistakes_coll/session/list?offset=${(paginationReactive.value.page-1)*paginationReactive.value.pageSize}&pagesize=${paginationReactive.value.pageSize}`).then(v=>{
    data.value=v.data
    ...
    LoadingSearch.value=false
        ...
        API.get(`/api/data/mistakes_coll/session/list/search/uuid?q=${encodeURIComponent(searchParams.value)}&offset=${(paginationReactive.value.page-1)*paginationReactive.value.pageSize}&pagesize=${paginationReactive.value.pageSize}`).then(v=>{
    data.value=v.data
    loadingData.value=false
    LoadingSearch.value=false
    })
